...
public class ExpenseTransactionProcessor implements TransactionProcessor {
    ...
    public Transaction create(Transaction transaction, String accountId) {
        transaction.setType(EXPENSE_TRANSACTION_TYPE);
        ...
        if (previousTransactions.isEmpty()) {
            ...
            transaction.setAccountBalance(previousTransactions.get(0).getAccountBalance().subtract(transaction.getAmount()));
        }
    ...
    public List<Transaction> getAll(final String accountId) {
        return transactionRepository.findByAccountIdAndType(accountId, EXPENSE_TRANSACTION_TYPE);
    }

    @Override
    public List<Transaction> getAllAfterDate(String accountId, Date date) {
        return transactionRepository
                .findByAccountIdAndType(accountId, EXPENSE_TRANSACTION_TYPE)
